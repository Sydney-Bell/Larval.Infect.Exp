...
pairwise_diff_ts <- combn(nrow(newdat_ts), 2, function(i) {
  ...
  data.frame(
    group1 = paste(newdat_ts[i[1], ], collapse = "_"),
    group2 = paste(newdat_ts[i[2], ], collapse = "_"),
    mean_diff = mean(diff_samples),
    lower = quantile(diff_samples, 0.025),
    upper = quantile(diff_samples, 0.975),
    pMCMC = 2 * min(mean(diff_samples > 0), mean(diff_samples < 0))
...
# Pairwise comparisons
##########
subset_df_cells$log1p_PercChangeCells <- log1p(subset_df_cells$PercChangeCells)

model_full <- MCMCglmm(
  log1p_PercChangeCells ~ SymbiontTreatment * Cross_2 * Treatment,
  random = ~ Replicate,
  data = subset_df_cells,
  family = "gaussian",
  prior = prior,
