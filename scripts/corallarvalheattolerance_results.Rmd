...
#Based on this result, there is a statistically significant difference in cell density among the different treatment, cell type, and Reef combinantions (Ambient, 31, and 35.5).

#pairwise Wilcoxon rank-sum test (pairwise comparisons)
...
#Based on this result, there is a statistically significant difference in cell density among the different treatments (Ambient, 31, and 35.5).
...
# 7. # Posterior predictions for marginal temperature effects
# 1. Compute posterior predictions for all combinations
posterior_preds <- X %*% t(model3.reef$Sol)

# Identify unique temperatures
temp_levels <- levels(df_with_extremes$Treatment)

# 2. Function to extract all posterior samples for a given temperature (all Symbiont × Cross)
get_post_samples <- function(temp, posterior_matrix, newdat) {
  rows <- which(newdat$Treatment == temp)
  posterior_matrix[rows, , drop = FALSE]  # keep full posterior for all rows
}

# 3. Compute marginal posterior summaries (mean + 95% CI)
marginal_summary <- data.frame(
  Temperature = temp_levels,
  post_mean = sapply(temp_levels, function(temp) mean(as.vector(get_post_samples(temp, posterior_preds, newdat.reef)))),
  lower_CI  = sapply(temp_levels, function(temp) quantile(as.vector(get_post_samples(temp, posterior_preds, newdat.reef)), probs = 0.025)),
  upper_CI  = sapply(temp_levels, function(temp) quantile(as.vector(get_post_samples(temp, posterior_preds, newdat.reef)), probs = 0.975))
)

# 4. Function to compute full posterior-based pMCMC for comparison vs reference
pMCMC_vs_ref <- function(temp, ref = temp_levels[1], posterior_matrix, newdat) {
  post_temp <- as.vector(get_post_samples(temp, posterior_matrix, newdat))
  post_ref  <- as.vector(get_post_samples(ref, posterior_matrix, newdat))
  
  diff <- post_temp - post_ref
  2 * min(mean(diff > 0), mean(diff < 0))  # two-sided pMCMC
}

# 5. Compute pMCMC for each temperature vs 27.5°C
marginal_summary$pMCMC_vs_27.5 <- sapply(temp_levels, function(temp) {
  if(temp == temp_levels[1]) return(NA)  # reference vs itself
  pMCMC_vs_ref(temp, ref = temp_levels[1], posterior_matrix = posterior_preds, newdat = newdat.reef)
})

# 6. Add significance labels
marginal_summary$signif <- case_when(
  marginal_summary$pMCMC_vs_27.5 < 0.005 ~ "**",
  marginal_summary$pMCMC_vs_27.5 < 0.05  ~ "*",
  TRUE                                   ~ ""
)

# 7. Ensure full precision is kept
options(digits = 8)

# 8. View results
marginal_summary

# 7b. Posterior predictions for marginal SymbiontTreatment effects
# 1. Compute posterior predictions for all combinations (already done)
posterior_preds <- X %*% t(model3.reef$Sol)

# Identify unique SymbiontTreatment levels
sym_levels <- levels(df_with_extremes$SymbiontTreatment)

# 2. Function to extract all posterior samples for a given SymbiontTreatment
get_post_samples_sym <- function(sym, posterior_matrix, newdat) {
  rows <- which(newdat$SymbiontTreatment == sym)
  posterior_matrix[rows, , drop = FALSE]  # keep full posterior for all rows
}

# 3. Compute marginal posterior summaries (mean + 95% CI)
marginal_sym_summary <- data.frame(
  SymbiontTreatment = sym_levels,
  post_mean = sapply(sym_levels, function(sym) mean(as.vector(get_post_samples_sym(sym, posterior_preds, newdat.reef)))),
  lower_CI  = sapply(sym_levels, function(sym) quantile(as.vector(get_post_samples_sym(sym, posterior_preds, newdat.reef)), probs = 0.025)),
  upper_CI  = sapply(sym_levels, function(sym) quantile(as.vector(get_post_samples_sym(sym, posterior_preds, newdat.reef)), probs = 0.975))
)

# 4. Function to compute full posterior-based pMCMC vs reference SymbiontTreatment
pMCMC_vs_ref_sym <- function(sym, ref = sym_levels[1], posterior_matrix, newdat) {
  post_sym <- as.vector(get_post_samples_sym(sym, posterior_matrix, newdat))
  post_ref <- as.vector(get_post_samples_sym(ref, posterior_matrix, newdat))
  
  diff <- post_sym - post_ref
  2 * min(mean(diff > 0), mean(diff < 0))  # two-sided pMCMC
}

# 5. Compute pMCMC for each SymbiontTreatment vs reference
marginal_sym_summary$pMCMC_vs_ref <- sapply(sym_levels, function(sym) {
  if(sym == sym_levels[1]) return(NA)  # reference vs itself
  pMCMC_vs_ref_sym(sym, ref = sym_levels[1], posterior_matrix = posterior_preds, newdat = newdat.reef)
})

# 6. Add significance labels
marginal_sym_summary$signif <- case_when(
  marginal_sym_summary$pMCMC_vs_ref < 0.005 ~ "**",
  marginal_sym_summary$pMCMC_vs_ref < 0.05  ~ "*",
  TRUE                                      ~ ""
)

# 7. View results
marginal_sym_summary

# 7c. Posterior predictions for marginal Cross_2 (Population) effects
# 1. posterior_preds already computed:
# posterior_preds <- X %*% t(model3.reef$Sol)

# Identify unique population levels
pop_levels <- levels(df_with_extremes$Cross_2)

# 2. Function to extract posterior draws for a given population
get_post_samples_pop <- function(pop, posterior_matrix, newdat) {
  rows <- which(newdat$Cross_2 == pop)
  posterior_matrix[rows, , drop = FALSE]
}

# 3. Marginal posterior summaries (mean + 95% CI)
marginal_pop_summary <- data.frame(
  Cross_2 = pop_levels,
  post_mean = sapply(pop_levels, function(pop)
    mean(as.vector(get_post_samples_pop(pop, posterior_preds, newdat.reef)))
  ),
  lower_CI = sapply(pop_levels, function(pop)
    quantile(as.vector(get_post_samples_pop(pop, posterior_preds, newdat.reef)),
             probs = 0.025)
  ),
  upper_CI = sapply(pop_levels, function(pop)
    quantile(as.vector(get_post_samples_pop(pop, posterior_preds, newdat.reef)),
             probs = 0.975)
  )
)

# 4. Posterior-based pMCMC for contrasts vs reference population
pMCMC_vs_ref_pop <- function(pop, ref = pop_levels[1], posterior_matrix, newdat) {
  post_pop <- as.vector(get_post_samples_pop(pop, posterior_matrix, newdat))
  post_ref <- as.vector(get_post_samples_pop(ref, posterior_matrix, newdat))
  
  diff <- post_pop - post_ref
  2 * min(mean(diff > 0), mean(diff < 0))  # two-sided
}

# 5. Compute pMCMC for each vs reference
marginal_pop_summary$pMCMC_vs_ref <- sapply(pop_levels, function(pop) {
  if(pop == pop_levels[1]) return(NA)
  pMCMC_vs_ref_pop(pop, ref = pop_levels[1],
                   posterior_matrix = posterior_preds, newdat = newdat.reef)
})

# 6. Add simple significance labels
marginal_pop_summary$signif <- case_when(
  marginal_pop_summary$pMCMC_vs_ref < 0.005 ~ "**",
  marginal_pop_summary$pMCMC_vs_ref < 0.05  ~ "*",
  TRUE                                      ~ ""
)
...
# 7. # Posterior predictions for marginal temperature effects
# 1. Compute posterior predictions for all combinations
posterior_preds <- X_s %*% t(mod.s.reef$Sol)
...
marginal_summary <- data.frame(
  Temperature = temp_levels,
  post_mean = sapply(temp_levels, function(temp) mean(as.vector(get_post_samples(temp, posterior_preds, newdat_size_reef)))),
  lower_CI  = sapply(temp_levels, function(temp) quantile(as.vector(get_post_samples(temp, posterior_preds, newdat_size_reef)), probs = 0.025)),
  upper_CI  = sapply(temp_levels, function(temp) quantile(as.vector(get_post_samples(temp, posterior_preds, newdat_size_reef)), probs = 0.975))
)
...
marginal_summary$pMCMC_vs_27.5 <- sapply(temp_levels, function(temp) {
  if(temp == temp_levels[1]) return(NA)  # reference vs itself
  pMCMC_vs_ref(temp, ref = temp_levels[1], posterior_matrix = posterior_preds, newdat = newdat_size_reef)
})
...
marginal_sym_summary <- data.frame(
  SymbiontTreatment = sym_levels,
  post_mean = sapply(sym_levels, function(sym) mean(as.vector(get_post_samples_sym(sym, posterior_preds, newdat_size_reef)))),
  lower_CI  = sapply(sym_levels, function(sym) quantile(as.vector(get_post_samples_sym(sym, posterior_preds, newdat_size_reef)), probs = 0.025)),
  upper_CI  = sapply(sym_levels, function(sym) quantile(as.vector(get_post_samples_sym(sym, posterior_preds, newdat_size_reef)), probs = 0.975))
)
...
marginal_sym_summary$pMCMC_vs_ref <- sapply(sym_levels, function(sym) {
  if(sym == sym_levels[1]) return(NA)  # reference vs itself
  pMCMC_vs_ref_sym(sym, ref = sym_levels[1], posterior_matrix = posterior_preds, newdat = newdat_size_reef)
})
...
marginal_pop_summary <- data.frame(
  Cross_2 = pop_levels,
  post_mean = sapply(pop_levels, function(pop)
    mean(as.vector(get_post_samples_pop(pop, posterior_preds, newdat_size_reef)))
  ),
  lower_CI = sapply(pop_levels, function(pop)
    quantile(as.vector(get_post_samples_pop(pop, posterior_preds, newdat_size_reef)),
             probs = 0.025)
  ),
  upper_CI = sapply(pop_levels, function(pop)
    quantile(as.vector(get_post_samples_pop(pop, posterior_preds, newdat_size_reef)),
...
marginal_pop_summary$pMCMC_vs_ref <- sapply(pop_levels, function(pop) {
  if(pop == pop_levels[1]) return(NA)
  pMCMC_vs_ref_pop(pop, ref = pop_levels[1],
                   posterior_matrix = posterior_preds, newdat = newdat_size_reef)
})
...
range(df_subset$PercChangeCells, na.rm=TRUE)
...
# 7. # Posterior predictions for marginal temperature effects
# 1. Compute posterior predictions for all combinations
posterior_preds <- X_subset %*% t(model_subset$Sol)

# Identify unique temperatures
temp_levels <- levels(totaldf_long$Treatment)
...
marginal_summary <- data.frame(
  Temperature = temp_levels,
  post_mean = sapply(temp_levels, function(temp) mean(as.vector(get_post_samples(temp, posterior_preds, newdat2_subset)))),
  lower_CI  = sapply(temp_levels, function(temp) quantile(as.vector(get_post_samples(temp, posterior_preds, newdat2_subset)), probs = 0.025)),
  upper_CI  = sapply(temp_levels, function(temp) quantile(as.vector(get_post_samples(temp, posterior_preds, newdat2_subset)), probs = 0.975))
)
...
marginal_summary$pMCMC_vs_27.5 <- sapply(temp_levels, function(temp) {
  if(temp == temp_levels[1]) return(NA)  # reference vs itself
  pMCMC_vs_ref(temp, ref = temp_levels[1], posterior_matrix = posterior_preds, newdat = newdat2_subset)
})
...
# Identify unique SymbiontTreatment levels
sym_levels <- levels(totaldf_long$SymbiontTreatment)
...
marginal_sym_summary <- data.frame(
  SymbiontTreatment = sym_levels,
  post_mean = sapply(sym_levels, function(sym) mean(as.vector(get_post_samples_sym(sym, posterior_preds, newdat2_subset)))),
  lower_CI  = sapply(sym_levels, function(sym) quantile(as.vector(get_post_samples_sym(sym, posterior_preds, newdat2_subset)), probs = 0.025)),
  upper_CI  = sapply(sym_levels, function(sym) quantile(as.vector(get_post_samples_sym(sym, posterior_preds, newdat2_subset)), probs = 0.975))
)
...
marginal_sym_summary$pMCMC_vs_ref <- sapply(sym_levels, function(sym) {
  if(sym == sym_levels[1]) return(NA)  # reference vs itself
  pMCMC_vs_ref_sym(sym, ref = sym_levels[1], posterior_matrix = posterior_preds, newdat = newdat2_subset)
})
...
# Identify unique population levels
pop_levels <- levels(totaldf_long$Cross_2)
...
marginal_pop_summary <- data.frame(
  Cross_2 = pop_levels,
  post_mean = sapply(pop_levels, function(pop)
    mean(as.vector(get_post_samples_pop(pop, posterior_preds, newdat2_subset)))
  ),
  lower_CI = sapply(pop_levels, function(pop)
    quantile(as.vector(get_post_samples_pop(pop, posterior_preds, newdat2_subset)),
             probs = 0.025)
  ),
  upper_CI = sapply(pop_levels, function(pop)
    quantile(as.vector(get_post_samples_pop(pop, posterior_preds, newdat2_subset)),
...
marginal_pop_summary$pMCMC_vs_ref <- sapply(pop_levels, function(pop) {
  if(pop == pop_levels[1]) return(NA)
  pMCMC_vs_ref_pop(pop, ref = pop_levels[1],
                   posterior_matrix = posterior_preds, newdat = newdat2_subset)
})
...
# pairwise for population
emm_pop <- emmeans(mod_betabin2, ~ host)

pairwise_pop <- pairs(emm_pop, adjust = "tukey")

pairwise_pop_df <- as.data.frame(pairwise_pop) %>%
  mutate(
    signif_label = case_when(
      p.value < 0.005 ~ "**",
      p.value < 0.05  ~ "*",
...
# pairwise for symbiont
emm_symb <- emmeans(mod_betabin2, ~ symbiont)

pairwise_symb <- pairs(emm_symb, adjust = "tukey")

pairwise_symb_df <- as.data.frame(pairwise_symb) %>%
  mutate(
    signif_label = case_when(
      p.value < 0.005 ~ "**",
      p.value < 0.05  ~ "*",
...
# Day 1 means collapsed across Cross_2 and SymbiontTreatment
day1_temp_df <- df_clean %>%
  filter(Timepoint == 1) %>%
  group_by(Treatment) %>%
  summarise(
    n = sum(!is.na(Area..mm.2.)),
...
# Ensure all 3 temperatures appear (fill missing with NA, not 0)
all_temps <- tibble(Treatment = factor(c("Ambient","31","35.5"),
                                      levels = c("Ambient","31","35.5")))

day1_temp_df_full <- all_temps %>%
  left_join(day1_temp_df, by = "Treatment")
...
# posterior draws for the contrast
diff_CC_31 <- pred_matrix_subset[6, ] - pred_matrix_subset[5, ]
diff_WW_31 <- pred_matrix_subset[8, ] - pred_matrix_subset[7, ]
...
# summary
mean(diff_CC_31)
quantile(diff_CC_31, c(0.025, 0.975))
pMCMC_CC_31

mean(diff_WW_31)
quantile(diff_WW_31, c(0.025, 0.975))
pMCMC_WW_31


# 7. # Posterior predictions for marginal temperature effects
# 1. Compute posterior predictions for all combinations
posterior_preds <- X_subset %*% t(model_perc$Sol)

# Identify unique temperatures
temp_levels <- levels(ipam$Treatment)
...
marginal_summary <- data.frame(
  Temperature = temp_levels,
  post_mean = sapply(temp_levels, function(temp) mean(as.vector(get_post_samples(temp, posterior_preds, newdat_perc)))),
  lower_CI  = sapply(temp_levels, function(temp) quantile(as.vector(get_post_samples(temp, posterior_preds, newdat_perc)), probs = 0.025)),
  upper_CI  = sapply(temp_levels, function(temp) quantile(as.vector(get_post_samples(temp, posterior_preds, newdat_perc)), probs = 0.975))
)
...
marginal_summary$pMCMC_vs_27.5 <- sapply(temp_levels, function(temp) {
  if(temp == temp_levels[1]) return(NA)  # reference vs itself
  pMCMC_vs_ref(temp, ref = temp_levels[1], posterior_matrix = posterior_preds, newdat = newdat_perc)
})
...
# Identify unique SymbiontTreatment levels
sym_levels <- levels(ipam$SymbiontTreatment)
...
marginal_sym_summary <- data.frame(
  SymbiontTreatment = sym_levels,
  post_mean = sapply(sym_levels, function(sym) mean(as.vector(get_post_samples_sym(sym, posterior_preds, newdat_perc)))),
  lower_CI  = sapply(sym_levels, function(sym) quantile(as.vector(get_post_samples_sym(sym, posterior_preds, newdat_perc)), probs = 0.025)),
  upper_CI  = sapply(sym_levels, function(sym) quantile(as.vector(get_post_samples_sym(sym, posterior_preds, newdat_perc)), probs = 0.975))
)
...
marginal_sym_summary$pMCMC_vs_ref <- sapply(sym_levels, function(sym) {
  if(sym == sym_levels[1]) return(NA)  # reference vs itself
  pMCMC_vs_ref_sym(sym, ref = sym_levels[1], posterior_matrix = posterior_preds, newdat = newdat_perc)
})
...
# Identify unique population levels
pop_levels <- levels(ipam$Cross)

# 2. Function to extract posterior draws for a given population
get_post_samples_pop <- function(pop, posterior_matrix, newdat) {
  rows <- which(newdat$Cross == pop)
  posterior_matrix[rows, , drop = FALSE]
}
...
marginal_pop_summary <- data.frame(
  Cross = pop_levels,
  post_mean = sapply(pop_levels, function(pop)
    mean(as.vector(get_post_samples_pop(pop, posterior_preds, newdat_perc)))
  ),
  lower_CI = sapply(pop_levels, function(pop)
    quantile(as.vector(get_post_samples_pop(pop, posterior_preds, newdat_perc)),
             probs = 0.025)
  ),
  upper_CI = sapply(pop_levels, function(pop)
    quantile(as.vector(get_post_samples_pop(pop, posterior_preds, newdat_perc)),
...
marginal_pop_summary$pMCMC_vs_ref <- sapply(pop_levels, function(pop) {
  if(pop == pop_levels[1]) return(NA)
  pMCMC_vs_ref_pop(pop, ref = pop_levels[1],
                   posterior_matrix = posterior_preds, newdat = newdat_perc)
})
