...
plotCul
```
# Create Figure 2 Heatmap with Source Reef
...
```
## Make a "mega heatmap" with all 5 traits
```{r}
...
```
## Combine PercentageWithCells and Cell Density for comparisons
```{r}
...
```
## Plot of mean FV/FM at T22 (first measurements taken at T4)
```{r}
# Filter the dataframe to get data for the final timepoint (22)
...
```
## Raw change in FV/FM for survivors
```{r}
# Group by Treatment, Cross_2, and SymbiontTreatment, and calculate mean and standard error for percent change
raw_change_fvfm_summary <- totaldf_long %>%
  filter(Timepoint == 22, Status == 0) %>%
...
```
## Combine the 3 plots for Fv/Fm into one figure
```{r}
# Combine the three plots into one figure
