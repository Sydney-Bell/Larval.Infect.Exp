...
Y <- df_with_extremes$PercChange
# SymbiontTreatment, Cross_2, Treatment are factors in your df

# ---- Step 2: Variance partitioning ----
varpart_result <- varpart(Y, ~SymbiontTreatment, ~Cross_2, ~Treatment, data = df_with_extremes)
...
# Unique Symbiont
rda_X1 <- rda(df_with_extremes$PercChange ~ SymbiontTreatment + Condition(Cross_2 + Treatment), 
              data = df_with_extremes)
anova_X1 <- anova(rda_X1, permutations = 999)

# Unique Cross
rda_X2 <- rda(df_with_extremes$PercChange ~ Cross_2 + Condition(SymbiontTreatment + Treatment), data = df_with_extremes)
anova_X2 <- anova(rda_X2, permutations = 999)

# Unique Treatment
rda_X3 <- rda(df_with_extremes$PercChange ~ Treatment + Condition(SymbiontTreatment + Cross_2), data = df_with_extremes)
...
ggsave("C:/Users/jc980786/OneDrive - James Cook University/GitHub/Larval.Infect.Exp/figures/FvFm_VariancePartition_Barplot.png", plot = p, width = 10, height = 6, dpi = 300)

write.csv(varpart_table, 
          file = "C:/Users/jc980786/OneDrive - James Cook University/GitHub/Larval.Infect.Exp/figures/FvFm_varpart_table.csv",
          row.names = FALSE)

___________________________________
library(MCMCglmm)

# Already fitted full model
# model_full <- MCMCglmm(PercChange ~ SymbiontTreatment * Cross_2 * Treatment, random = ~Replicate, ...)

# Fit single fixed effect models
model_symbiont <- MCMCglmm(PercChange ~ SymbiontTreatment, random = ~Replicate, data = df_with_extremes, family = "gaussian", prior = prior, nitt=50000, burnin=10000, thin=20)
model_cross <- MCMCglmm(PercChange ~ Cross_2, random = ~Replicate, data = df_with_extremes, family = "gaussian", prior = prior, nitt=50000, burnin=10000, thin=20)
model_treatment <- MCMCglmm(PercChange ~ Treatment, random = ~Replicate, data = df_with_extremes, family = "gaussian", prior = prior, nitt=50000, burnin=10000, thin=20)

# ---- 2. Function to compute marginal R² per posterior sample ----
calc_R2_samples <- function(model) {
  linpred <- model$X %*% t(model$Sol)  # n x iterations
  var_fixed <- apply(linpred, 2, var)
  var_random <- rowSums(model$VCV[, "Replicate", drop = FALSE])
  var_resid <- model$VCV[, "units"]
  R2 <- var_fixed / (var_fixed + var_random + var_resid)
  return(R2)  # vector of posterior R² values
}

# ---- 3. Compute posterior R² samples ----
R2_full_samples <- calc_R2_samples(model_full)
R2_symb_samples <- calc_R2_samples(model_symbiont)
R2_cross_samples <- calc_R2_samples(model_cross)
R2_treat_samples <- calc_R2_samples(model_treatment)

# ---- 4. Compute unique, shared, and residual fractions per posterior sample ----
unique_symb_samples <- R2_symb_samples
unique_cross_samples <- R2_cross_samples
unique_treat_samples <- R2_treat_samples
shared_samples <- R2_full_samples - (unique_symb_samples + unique_cross_samples + unique_treat_samples)
residual_samples <- 1 - R2_full_samples

# Combine into a dataframe for plotting / summary
varpart_samples <- data.frame(
  Unique_Symbiont = unique_symb_samples,
  Unique_Cross = unique_cross_samples,
  Unique_Treatment = unique_treat_samples,
  Shared = shared_samples,
  Residual = residual_samples
)

# ---- 5. Summarize (mean + 95% credible intervals) ----
varpart_summary <- varpart_samples %>%
...
ggplot(varpart_summary, aes(x = Component, y = mean, fill = Component)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  theme_minimal(base_size = 14) +
  ylab("Variance explained (posterior mean ± 95% CI)") +
  xlab("") +
  ggtitle("Bayesian Variance Partitioning") +
  theme(legend.position = "none")

##################### interactions
# Main effects
main_effects <- c("SymbiontTreatment", "Cross_2", "Treatment")

# Two-way interactions
interactions_2way <- c("SymbiontTreatment:Cross_2",
                        "SymbiontTreatment:Treatment",
                        "Cross_2:Treatment")

# Three-way interaction
interaction_3way <- "SymbiontTreatment:Cross_2:Treatment"

library(MCMCglmm)

# Prior (same as your existing workflow)
prior <- list(R = list(V = 1, nu = 0.002), G = list(G1 = list(V = 1, nu = 0.002)))

# Function to fit model and compute marginal R²
compute_marginal_R2 <- function(fixed_formula, data, random = ~Replicate) {
  mod <- MCMCglmm(fixed = fixed_formula,
                  random = random,
                  data = data,
                  family = "gaussian",
                  prior = prior,
  ...
  Sol <- mod$Sol[, colnames(X), drop = FALSE]
  
  linpred <- X %*% t(Sol)
  var_fixed <- apply(linpred, 2, var)
  
  var_random <- rowSums(mod$VCV[, grep("Replicate", colnames(mod$VCV)), drop = FALSE])
  var_resid <- mod$VCV[, "units"]
  
  R2 <- var_fixed / (var_fixed + var_random + var_resid)
  
  c(mean = mean(R2), lower = quantile(R2, 0.025), upper = quantile(R2, 0.975))
}

# Main effects
r2_main <- lapply(main_effects, function(term) {
  compute_marginal_R2(as.formula(paste("PercChange ~", term)), df_with_extremes)
})
names(r2_main) <- main_effects

# Two-way interactions
# List your 2-way interactions as strings
interactions_2way <- c(
  "SymbiontTreatment:Cross_2",
  "SymbiontTreatment:Treatment",
  "Cross_2:Treatment"
)

# Helper function to compute marginal R² for any formula
compute_marginal_R2 <- function(model, data, fixed_formula) {
...
# Apply function to all 2-way interactions
r2_2way <- lapply(interactions_2way, function(term) {
  compute_marginal_R2(term, df_with_extremes, model_full)
})

names(r2_2way) <- interactions_2way
r2_2way


# Three-way interaction
r2_3way <- compute_marginal_R2(interaction_3way, df_with_extremes, model_full)
r2_3way

results_df <- rbind(
  do.call(rbind, r2_main),
  do.call(rbind, r2_2way),
  ThreeWay = r2_3way
)

results_df <- as.data.frame(results_df)
results_df$Component <- rownames(results_df)

# Optional: compute % of full model
full_model_R2 <- compute_marginal_R2(
  model = model_full,
  data = df_with_extremes,
  fixed_formula = PercChange ~ SymbiontTreatment * Cross_2 * Treatment
)

results_df$Relative_Contribution <- results_df$mean / full_model_R2["mean"]
results_df$Relative_Contribution_Percent <- round(100 * results_df$Relative_Contribution, 1)

colnames(results_df)[colnames(results_df) == "lower.2.5%"] <- "lower"
colnames(results_df)[colnames(results_df) == "upper.97.5%"] <- "upper"

results_df <- results_df[, c("Component", "mean", "lower", "upper", "Relative_Contribution_Percent")]

library(ggplot2)

ggplot(results_df, aes(x = Component, y = mean, fill = Component)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_text(aes(label = paste0(round(mean, 3), " (", round(lower,3), "-", round(upper,3), ")")),
            vjust = -0.5, size = 3) +
  theme_minimal(base_size = 14) +
  ylab("Proportion of variance explained (marginal R²)") +
  xlab("") +
  ggtitle("Variance Partitioning: Main Effects vs Interactions") +
  theme(legend.position = "none")
...
# ---- Step 1: Define predictors and response ----
Y <- df_with_extremes$PercChangeSize
# SymbiontTreatment, Cross_2, Treatment are factors in your df
...
# ---- Step 3: RDA + permutation tests for unique fractions ----
# Unique Symbiont
rda_X1 <- rda(df_with_extremes$PercChangeSize ~ SymbiontTreatment + Condition(Cross_2 + Treatment), 
              data = df_with_extremes)
anova_X1 <- anova(rda_X1, permutations = 999)

# Unique Cross
rda_X2 <- rda(df_with_extremes$PercChangeSize ~ Cross_2 + Condition(SymbiontTreatment + Treatment), data = df_with_extremes)
anova_X2 <- anova(rda_X2, permutations = 999)

# Unique Treatment
rda_X3 <- rda(df_with_extremes$PercChangeSize ~ Treatment + Condition(SymbiontTreatment + Cross_2), data = df_with_extremes)
anova_X3 <- anova(rda_X3, permutations = 999)
...
ggsave("C:/Users/jc980786/OneDrive - James Cook University/GitHub/Larval.Infect.Exp/figures/Size_VariancePartition_Barplot.png", plot = p, width = 10, height = 6, dpi = 300)

write.csv(varpart_table, 
          file = "C:/Users/jc980786/OneDrive - James Cook University/GitHub/Larval.Infect.Exp/figures/Size_varpart_table.csv",
          row.names = FALSE)
```
...
# ---- Step 1: Define predictors and response ----
Y <- df_subset$PercChangeCells
# SymbiontTreatment, Cross_2, Treatment are factors in your df

# ---- Step 2: Variance partitioning ----
varpart_result <- varpart(Y, ~SymbiontTreatment, ~Cross_2, ~Treatment, data = df_subset)
...
# ---- Step 3: RDA + permutation tests for unique fractions ----
# Unique Symbiont
rda_X1 <- rda(df_subset$PercChangeCells ~ SymbiontTreatment + Condition(Cross_2 + Treatment), 
              data = df_subset)
anova_X1 <- anova(rda_X1, permutations = 999)

# Unique Cross
rda_X2 <- rda(df_subset$PercChangeCells ~ Cross_2 + Condition(SymbiontTreatment + Treatment), data = df_subset)
anova_X2 <- anova(rda_X2, permutations = 999)

# Unique Treatment
rda_X3 <- rda(df_subset$PercChangeCells ~ Treatment + Condition(SymbiontTreatment + Cross_2), data = df_subset)
anova_X3 <- anova(rda_X3, permutations = 999)
...
ggsave("C:/Users/jc980786/OneDrive - James Cook University/GitHub/Larval.Infect.Exp/figures/Density_VariancePartition_Barplot.png", plot = p, width = 10, height = 6, dpi = 300)

write.csv(varpart_table, 
          file = "C:/Users/jc980786/OneDrive - James Cook University/GitHub/Larval.Infect.Exp/figures/Density_varpart_table.csv",
          row.names = FALSE)
```
